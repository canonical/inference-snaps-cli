...
func TestUnixSocketNameInNonUnixProtocol(t *testing.T) {
	manifest := templateManifest()
	manifest.Servers = map[string]Server{
		"openai": {
			Protocol:   "http",
			UnixSocket: "socket.sock",
		},
	}

	err := manifest.validate("test")
	if err == nil {
		t.Fatal("unix-socket in non-unix protocol should be invalid")
	}
}
func TestUnixSocketNameIsFileName(t *testing.T) {
	manifest := templateManifest()
	manifest.Servers = map[string]Server{
		"openai": {
			Protocol:   "http+unix",
	...
	err := manifest.validate("test")
	if err == nil {
		t.Fatal("unix-socket with a path should be invalid")
	}
}

func TestUnixSocketValid(t *testing.T) {
	manifest := templateManifest()
	server := Server{
		Protocol:   "http+unix",
		UnixSocket: "server.sock",
	}
	manifest.Servers = map[string]Server{
		"openai": server,
	}
	...
	err := manifest.validate("test")
	if err != nil {
		t.Fatalf("unix-socket %q for protocol %q should be valid, got error: %v",
			server.UnixSocket, server.Protocol, err)
	}
}
